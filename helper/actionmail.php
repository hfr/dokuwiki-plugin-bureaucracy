...
class helper_plugin_bureaucracy_actionmail extends helper_plugin_bureaucracy_action
{
    ...
    public function run($fields, $thanks, $argv)
    {
        ...
        $event = new Event('PLUGIN_BUREAUCRACY_EMAIL_SEND', $evdata);
        if ($event->advise_before()) {
            //set default subject
            $this->subject = sprintf($this->getLang('mailsubject'), $ID);

            //build html&text table, collect replyto and subject
            [$table_html, $table_text] = $this->processFieldsBuildTable($fields, $mail);
            ...
            if ($this->mailtemplate) {
                ...
                [$this->_mail_html, $this->_mail_text] = $this->getContent();
            } else {
                //show simpel listing
        ...
        foreach ($fields as $field) {
            ...
            switch ($field->getFieldType()) {
                    ...
                    if (!empty($field->depends_on)) {
                        ...
                        foreach ($fields as $field_tmp) {
                            if ($field_tmp->getParam('label') === $field->depends_on[0] && $field_tmp->getParam('value') === $field->depends_on[1]) {
                                [$html, $text] =  $this->mail_buildRow($label);
                            }
                        }
                    } else {
                        [$html, $text] =  $this->mail_buildRow($label);
                ...
                case 'file':
                    ...
                    } elseif ($file['size'] > $this->getConf('maxEmailAttachmentSize')) {
                    ...
                    [$html, $text] = $this->mail_buildRow($label, $message);
                    break;
                case 'subject':
                    $this->subject = $label;
                    break;
                ...
                default:
                    ...
                    [$html, $text] = $this->mail_buildRow($label, $value);
    ...
    protected function getContent()
    {
        ...
        if (preg_match_all('#<code\b(.*?)>(.*?)</code>#is', $content, $matches)) {
            foreach ($matches[1] as $index => $codeoptions) {
                [$syntax, ] = explode(' ', trim($codeoptions), 2);
                if ($syntax == 'html') {
        ...
        return [$this->replace($html), $this->replace($text)];
    }
}
